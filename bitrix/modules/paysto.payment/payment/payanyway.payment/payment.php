...
$url_paysto_pay = $is_demo ? 'https://demo.moneta.ru/assistant.htm' : 'https://moneta.ru/assistant.htm';
...
$MNT_DESCRIPTION = urlencode(GetMessage('SALE_PAYSTO_ORDER_ID') . ' #' . $arOrder['ID']);
...
// link to pay
$link = $url_paysto_pay . '?MNT_ID=' . $MNT_ID . '&MNT_TRANSACTION_ID=' . $MNT_TRANSACTION_ID . '&MNT_CURRENCY_CODE=' . $MNT_CURRENCY_CODE . '&MNT_AMOUNT=' . $MNT_AMOUNT . '&MNT_TEST_MODE=' . $MNT_TEST_MODE . '&MNT_SUCCESS_URL = ' . $MNT_SUCCESS_URL . '&MNT_FAIL_URL=' . $MNT_FAIL_URL . '&MNT_SIGNATURE=' . $MNT_SIGNATURE . '&MNT_DESCRIPTION=' . $MNT_DESCRIPTION;
if ($user_email) {
...
$message = str_replace(array('{$order_id}', '{$amount}', '{$link}'), array($arOrder['ID'], $arOrder['PRICE'], $link), file_get_contents(dirname(__FILE__) . '/paysto_mail.html'));
